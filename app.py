...
@app.route('/choose_stage_id/<username>/<id_link>') #khi chưa chọn bài học
def choose_stage_id(username, id_link):
    user = User.query.filter_by(username=username).first()
    selected_stage = id_link
    ...
    if stage_info is None:
        message = "Không tìm thấy thông tin giai đoạn tương ứng."
        session['message'] = message
        return redirect(url_for('choose_stage', username=username))  # Redirect to choose_stage

    elif stage_info[3] == 0:
        message = "Khóa học đang được xây dựng, xin quý khách vui lòng chọn khóa học khác."
        session['message'] = message
        return redirect(url_for('choose_stage', username=username))  # Redirect to choose_stage
        ...
        db.session.commit()
        print("debut: , hoho")
        return redirect(url_for('user_view', username=username))  # Redirect to user_view
